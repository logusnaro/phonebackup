...
public sealed class ContactService
{
    ...
    public async Task<IReadOnlyList<Contact>> ListAsync(Guid? memberId = null)
    {
        var rows = await _database.QueryAsync("""
            SELECT id,member_id,display_name,company,notes,updated_at FROM contacts
            WHERE deleted_at IS NULL AND member_id=$member
            ORDER BY display_name
            """, r => new { Id = Guid.Parse(r.GetString(0)), MemberId = r.IsDBNull(1) ? (Guid?)null : Guid.Parse(r.GetString(1)), Name = r.GetString(2), Company = r.IsDBNull(3) ? null : r.GetString(3), Notes = r.IsDBNull(4) ? null : r.GetString(4), Updated = DateTimeOffset.Parse(r.GetString(5)) },
            p => p.AddWithValue("$member", (object?)memberId?.ToString() ?? DBNull.Value));
        ...
        foreach (var row in rows)
        {
            ...
            contacts.Add(new Contact(row.Id, row.Name, phones, emails, row.Company, row.Notes, row.Updated, row.MemberId));
        }
        return contacts;
    }
    ...
    public async Task SaveAsync(Contact contact)
    {
        await _database.ExecuteAsync("""
            INSERT INTO contacts(id,member_id,display_name,company,notes,updated_at,deleted_at) VALUES($id,$member,$name,$company,$notes,$at,NULL)
            ON CONFLICT(id) DO UPDATE SET member_id=excluded.member_id,display_name=excluded.display_name,company=excluded.company,notes=excluded.notes,updated_at=excluded.updated_at,deleted_at=NULL;
            DELETE FROM contact_phones WHERE contact_id=$id; DELETE FROM contact_emails WHERE contact_id=$id;
            """, p => { p.AddWithValue("$id", contact.Id.ToString()); p.AddWithValue("$member", (object?)contact.MemberId?.ToString() ?? DBNull.Value); p.AddWithValue("$name", contact.DisplayName); p.AddWithValue("$company", (object?)contact.Company ?? DBNull.Value); p.AddWithValue("$notes", (object?)contact.Notes ?? DBNull.Value); p.AddWithValue("$at", DateTimeOffset.UtcNow.ToString("O")); });
